...
{// 主要以前者为判断，后者好像现在没设置了
	return f->pict_type==FF_I_TYPE || f->key_frame==1;
}
//////////////////////////////////////////////////////////////////////////
//
//  Public properties
...
void FFmpegEncoder::open(const char *fileName, bool openCodec)
{
    ...
    if (encodeVideo)
    {
        ...
        if ((!outputFormat || outputFormat->video_codec == CODEC_ID_NONE) && videoParam.codecId == CODEC_ID_NONE)
        {
    ...
    if (encodeAudio)
    {
        ...
        if ((!outputFormat || outputFormat->audio_codec == CODEC_ID_NONE) && audioParam.codecId == CODEC_ID_NONE)
        {
  ...
  // avcodec_get_frame_defaults(&frame);
	...
	if (!ret && got_packet && videoPkt->size) {
		...
		{// B帧这样丢了也有问题！是不是要把之后所以B帧都给丢了？
			if (!(bCount++ % 2)){printf("drop bFrame %d\n", bCount);
				return 0;
			}
		}
		/* 不能丢失P帧否则马赛克N多
		...
		if(videoPkt->flags & PKT_FLAG_KEY)
			reqKeyFrame = 0;
		else if(reqKeyFrame){
